...
static void mainHandler (union sigval handlerParam){
	...
	struct Timer *theTimer = handlerParam.sival_ptr;

	// Calling the callback function
	theTimer->callback(theTimer->bomb);
}

extern struct Timer *Timer_new (uint32_t thisDelay,
				Timer_callback callback,
				void * bomb){
	struct Timer * newInstance;
	int error;

	// allocates and initializes the watchdog's attributes
	newInstance = (struct Timer *) malloc(sizeof(struct Timer));
	if (newInstance == NULL) {
...
extern void Timer_start (struct Timer * this){
	...
	// Seconds
	time.it_value.tv_sec = this->delay / TIME_UNIT;
	// Nanoseconds
	time.it_value.tv_nsec = (this->delay % TIME_UNIT) * (1000000000 / TIME_UNIT);
